$\Theta$ think of upper and bottom bounding by some constant. So f(n) = $\Theta$(g(n)) if there exists some constants $c_1$ and $c_2$ such that $c_1\cdot g(n) \leq f(n) \leq c_2\cdot g(n)$ 
